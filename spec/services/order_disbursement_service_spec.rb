  ...
  describe '#calculate_disbursement' do
    context 'when valid' do
      it 'creates a disbursement' do
        expect { OrderDisbursementService.new(Date.today, merchant.id, orders.pluck(:id)).disburse }.to change { Disbursement.count }.by(1)
      end
      ...
      it 'creates a disbursement with sum of orders net_amount' do
        ...
        OrderDisbursementService.new(Date.today, merchant.id, orders.pluck(:id)).disburse

        expect(Disbursement.last.net_amount).to eq(net_amount)
      end

      it 'should disburse order' do
        OrderDisbursementService.new(Date.today, merchant.id, [order.id]).disburse
        expect(order.reload.disbursed).to eq(true)
      end

      it 'should not disburse a disbursed order' do
        OrderDisbursementService.new(Date.today, merchant.id, [disbursed_order]).disburse
    ...
    context 'when invalid' do
      it 'returns an error if the disbursement is not created' do
        ...
        service = OrderDisbursementService.new(Date.today, merchant.id, orders)
