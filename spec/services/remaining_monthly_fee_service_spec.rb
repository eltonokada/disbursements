  ...
  describe '#calculate' do
    ...
    context 'when valid' do
      it 'creates a monthly fee when minimum monthly fee not achieved' do
        RemainingMonthlyFeeService.new(Date.today, merchant).calculate
        expect(RemainingMonthlyFee.count).to eq(1)
      end
      it 'creates a monthly fee when remaining fee 195' do
        RemainingMonthlyFeeService.new(Date.today, merchant_without_remaining_fee).calculate
    ...
    context 'when invalid' do
      it 'logs an error if record creation fails' do
        ...
        described_class.new(Date.today, merchant).calculate
      end
    end
  end
end
